...
On continue en regardant l'évolution entre 2014 et 2018 du nombre d'HComp par étudiant pour tous les établissements, par rapport à la moyenne.

```{r evol.hcomp}
df <- hec %>%
  mutate(
    hecPetu = HC.Total / (kpi.ETU.S.cycle.1.L + kpi.ETU.S.cycle.2.M),
    hecPetu.norm = hecPetu / mean(hecPetu, na.rm=TRUE)
  ) %>%
  filter(Rentrée > 2012)

ggplot(df, aes(y = hecPetu.norm, x = Rentrée,
           color=(Etablissement),alpha=Rentrée)) +
  geom_point()+
  geom_line()+
  geom_text_repel(data=filter(df,Rentrée == 2018), aes(label=Etablissement), hjust=0, direction="y") +
  xlab("Annee")+
