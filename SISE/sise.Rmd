...
read.sise.2019 <- function(file, rentrée.max = Inf) {
  vroom(file, delim = ";", locale = locale(decimal_mark = ","), .name_repair = "universal") %>%
...
read.sise.2024 <- function(file, rentrée.max = Inf) {
  vroom(file, delim = ";", locale = locale(decimal_mark = ","), .name_repair = "universal") %>%
    filter(rentree < rentrée.max) %>%
    group_by(
      Rentrée = rentree,
