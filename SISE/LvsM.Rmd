...
sise %>%
  filter(Niveau == "XX", LMD != "D", Rentrée==2024) %>%
  group_by(
...
sise %>%
  ...
  ggplot_sd(aes(x=Rentrée, y= Etudiants, color=Niveau)) +
...
```{r l3m1.decal}
sise %>%
  # filter(Diplôme != "Master enseignement") %>%
  ...
  filter(Rentrée != 2012, Rentrée != 2025) %>%
  ggplot_sd(aes(x=Rentrée, y= Etudiants, color=Niveau, shape=Niveau)) +
  geom_line(linewidth = 1.5) + geom_point(stroke = 2, size = 2, fill="white") + 
...
animate(flux, start_pause = 20, end_pause = 20)
```
 ...
 export <- sise %>%
  ...
  group_by(
    ...
    Niveau = case_when(
      ...
      LMD == "M" & Niveau == "01" & Diplôme == "Masters hors enseignement" ~ "M1_autres",
      TRUE ~ "Autres")
  ) %>%
...
sise %>%
  ...
  mutate(Rentrée = ifelse(Niveau == "L3", Rentrée+2,Rentrée)) %>%
  mutate(Rentrée = ifelse(Niveau == "M1", Rentrée+1,Rentrée)) %>%
  #filter(Rentrée != 2012, Rentrée != 2023) %>%
  ggplot(aes(x=Rentrée, y= Etudiants, color=Niveau)) +
  geom_line(linewidth = 1.5) + geom_point(shape = 21, stroke = 2, size = 2, fill="white") + 
  scale_x_continuous(labels = ~ paste0("L3 ", .x-2,"\nM1 ",.x), breaks = seq(2011,2030,2), name="Cohorte") +
  scale_y_continuous(labels = ~ paste0(.x/1e3,"k"), name = "Inscriptions étudiantes") +
  ggtitle("Etudiant⋅e⋅s  inscrit⋅e⋅s en L3 à l'année N et M1 à l'année N+1" )
```
