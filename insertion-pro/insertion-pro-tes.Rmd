...
ipmn %>%
  ...
  mutate(diff=Taux.d.emploi.salarié.en.France-Taux.d.insertion) %>%
  pivot_taux() %>%
  mutate(Indicateur.short = factor(case_match(Indicateur,
                                       "Taux d'insertion" ~ "TI",
                                       "Taux d'emploi" ~ "TE",
                                       .default = "TESF"), levels = c("TI","TE","TESF"))) %>%
  ggplot(aes(x=Indicateur.short,y=Valeur,fill=Indicateur, color=Ensemble)) +
  geom_col(alpha=0.8) +
  scale_x_discrete(name="") +
  scale_y_continuous(labels = ~ scales::percent(.x/100), name="Taux") +
  scale_color_manual(values=c("white","black"), guide=FALSE) +
  facet_grid(.~Code.du.domaine) +
  expand_limits(y=100) +
...
```{r, eval=FALSE}
sise.socio <- vroom("../data/fr-esr-sise-effectifs-d-etudiants-inscrits-esr-public.csv", delim = ";", locale = locale(decimal_mark = ","), 
        .name_repair = "universal",
        col_select = c(rentree,Type.d.établissement,Etablissement,etablissement_id_paysage_actuel,Diplôme,Grande.discipline,Secteur.disciplinaire,sexe,bac,Série.de.baccalauréat,dn_de,ESPE...INSPE,Attractivité.internationale,cursus_lmd,niveau,Nombre.d.étudiants.inscrits..inscriptions.principales..hors.étudiants.inscrits.en.parallèle.en.CPGE,Nombre.total.d.étudiants.inscrits..inscriptions.principales.et.secondes..hors.étudiants.inscrits.en.parallèle.en.CPGE)) %>%
    ...
    group_by(
      ...
      Niveau = niveau #= ifelse(DIPLOME == "LIC_PRO", "3", NIVEAU)
    ) %>%
    summarise(
...
save(sise.socio, file="sise_socio.RData")
```

```{r}
load("sise_socio.RData")
...
sise.taux <- full_join(
  ...
  sise2taux("Série.de.baccalauréat","Baccalauréat général","BacGénéral") ) 
