...
t208 %>%
  ...
  ggplot(aes(x=ANNEE5, y=TXEMPBIT, color=DDIPL, group=DDIPL)) +
    ...
    theme_cpesr() + guides(color=guide_legend(ncol=3, title = "")) +
...
pop.all <- bind_rows(
 ...
 pop.all %>%
  ggplot(aes(x=Année, y=Valeur, color=Population, group=Population)) +
    geom_line() +
    theme_cpesr() + guides(color=guide_legend(ncol=2, title = ""))
```
 ...
 pop.all %>%
  ...
  ggplot(aes(x=Année, y=Valeur, fill=Population, group=Population)) +
   ...
   geom_line(data=pop.all %>% filter(Population == "Ensemble des 15-29") ) +
    theme_cpesr() + guides(fill=guide_legend(ncol=2, title = ""), color=FALSE)
```
...
saphire %>%
  ...
  expand_limits(y=0) +
  theme_cpesr()  + guides(fill=guide_legend(ncol=3, title = ""), color=guide_legend(ncol=3, title = "")) +
...
saphire %>%
  ...
  ggplot(aes(x=Année,y=Population,fill=Diplôme, color=Diplôme)) +
    ...
    expand_limits(y=0) +
    theme_cpesr()  + guides(fill=guide_legend(ncol=3, title = ""), color=guide_legend(ncol=2, title = "")) +
...
saphire %>%
  ...
  geom_area(aes(group=Diplôme), color="white", alpha=0.5) +
  expand_limits(y=0) +
  theme_cpesr()  + guides(fill=guide_legend(ncol=2, title = ""))
```
