...
- Activité, emploi et chômage en 2020 et en séries longues (T101 et T208) : https://www.insee.fr/fr/statistiques/5359500?sommaire=5359511
...
saphire.var <- read.csv2("Varmod_RP_1968-2018.csv")
...
saphire <- data.table::fread("FD_RP_1968-2018.csv", header = T) %>%
  tibble() %>%
...
saphire %>%
  ...
  theme_cpesr_cap() +
  ylab("Population (milliers)") +
  ggtitle(paste("Population française par activité et âge",saphire.last.année))
```

```{r saphire.allpop.last.col}
saphire %>%
  ungroup() %>%
  filter(Année == max(Année),Age >=15, Age <=100) %>%
  group_by(Age,Activité) %>%
...
```{r saphire.1629.act.data, results='asis'}
saphire.1629 %>%
  bind_rows(
    saphire.1629 %>%
...
```{r saphire.sim.data, results='asis'}
saphire.sim %>% 
  arrange(Année,Activité) %>%
  group_by(Année) %>%
  mutate(Part = scales::percent(Population / sum(Population))) %>%
  spoilerTable()
...
saphire %>%
  ...
  theme_cpesr_cap() +
  theme(axis.title.x = element_blank()) +
  ggtitle("Activité des jeunes français⋅es entre 16 en 24 ans, de 1968 à nos jours")
```
...
saphire %>%
  ...
  theme_cpesr_cap() +
  theme(axis.title.x = element_blank()) +
  ggtitle("Activité des jeunes français⋅es entre 25 en 29 ans, de 1968 à nos jours")
```
...
```{r saphire.dipl.data, results='asis'}
saphire %>%
  filter(Diplôme != "Personnes de moins de 15 ans (17 ans pour le RP 1975) ou étudiants - élèves") %>%
  mutate(Diplôme = fct_drop(Diplôme)) %>%
  group_by(Année,Diplôme) %>%
...
saphire %>%
  ...
  ggplot(aes(x=Année,y=Population,fill=Diplôme, color=Diplôme)) +
    ...
    ggtitle("Répartition par diplôme de la population française", subtitle="(hors élèves et étudiants)") +
...
saphire %>%
  ...
  ggplot(aes(x=Année,y=Population,fill=Diplôme, color=Diplôme)) +
    ...
    ylab("Population (pourcentage)") +
    theme(axis.title.x = element_blank()) # + labs(caption="Julien Gossa - SNESUP-FSU  Source : base SAPHIRE INSEE\nhttps://github.com/cpesr/RFC/blob/main/population/population.md")
```

```{r saphire.dipl.16.29.pyramide, fig.height=12, fig.asp=6/4}
saphire %>%
  filter(Diplôme != "Personnes de moins de 15 ans (17 ans pour le RP 1975) ou étudiants - élèves") %>%
  mutate(Diplôme = fct_drop(Diplôme)) %>%
  filter(Age >= 25, Age <35) %>%
  group_by(Année,Diplôme,Sexe) %>%
  summarise(Population= round(sum(Population) / 1000)) %>%
  ungroup() %>%
  #complete(Année,Diplôme,Sexe,fill=list(Population=0)) %>% 
  #mutate(Diplôme = fct_rev(Diplôme)) %>%
  mutate(Population = ifelse(Sexe=="Homme",-Population,Population)) %>%
  ggplot(aes(x=Diplôme,y=Population,fill=Sexe)) +
    geom_col( alpha=0.5, position="identity") +
    #scale_y_continuous(labels=scales::percent) +
    coord_flip() +
    facet_grid(Année~.,scales = "free_y") +
    theme_cpesr_cap()  + guides(fill=guide_legend(ncol=3, title = ""), color=guide_legend(ncol=2, title = "")) +
    ggtitle("Répartition par diplôme de la population française entre 25 à 34 ans", subtitle="(hors élèves et étudiants)") +
    ylab("Population (pourcentage)") +
    theme(axis.title.x = element_blank()) # + labs(caption="Julien Gossa - SNESUP-FSU  Source : base SAPHIRE INSEE\nhttps://github.com/cpesr/RFC/blob/main/population/population.md")
...
```{r saphire.dipl.16.29.pyramide2, fig.height=12, fig.asp=4/4}
saphire %>%
  filter(Diplôme != "Personnes de moins de 15 ans (17 ans pour le RP 1975) ou étudiants - élèves") %>%
  mutate(Diplôme = fct_drop(Diplôme)) %>%
  mutate(Diplôme = fct_recode(Diplôme,
                              "Aucun diplôme - CEP - BEPC" = "Aucun diplôme",
                              "Aucun diplôme - CEP - BEPC" = "Niveau CEP", 
                              "Aucun diplôme - CEP - BEPC" = "Niveau BEPC")) %>%
  filter(Age >= 25, Age <35) %>%
  group_by(Année,Diplôme,Sexe) %>%
  summarise(Population= round(sum(Population) / 1000)) %>%
  ungroup() %>%
...
```{r saphire.dipl.16.29.pyramide2.gif, cache=TRUE, fig.asp=9/16}
saphire %>%
  filter(Diplôme != "Personnes de moins de 15 ans (17 ans pour le RP 1975) ou étudiants - élèves") %>%
  mutate(Diplôme = fct_drop(Diplôme)) %>%
  mutate(Diplôme = fct_recode(Diplôme,
                              "Diplôme universitaire de 1er cycle" = "Diplôme supérieur au Baccalauréat",
                              "Aucun diplôme - CEP - BEPC" = "Aucun diplôme",
                              "Aucun diplôme - CEP - BEPC" = "Niveau CEP", 
                              "Aucun diplôme - CEP - BEPC" = "Niveau BEPC")) %>%
  filter(Age >= 25, Age <35) %>%
  ...
  ggplot(aes(x=Diplôme,y=Population,fill=Sexe)) +
    ...
    coord_flip() +
    theme_cpesr_cap()  + guides(fill=guide_legend(ncol=3, title = ""), color=guide_legend(ncol=2, title = "")) +
    ggtitle("Répartition par diplôme en {closest_state}", subtitle="Population française entre 25 à 34 ans") +
    ylab("Population (pourcentage)") +
    theme(axis.title.x = element_blank()) +
    gganimate::transition_states(Année) 
```

```{r saphire.dipl.16.29.data, results='asis'}
saphire %>%
  filter(Diplôme != "Personnes de moins de 15 ans (17 ans pour le RP 1975) ou étudiants - élèves") %>%
  mutate(Diplôme = fct_drop(Diplôme)) %>%
  filter(Age >= 25, Age <35) %>%
...
```{r saphire.allpop.gif.col.amort, cache=TRUE, fig.asp=9/16}
library(gganimate)
...
saphire %>%
  ...
  filter(Age >=16, Age <=29) %>%
  group_by(Année,Age,Activité) %>%
  mutate(Activité=factor(Activité, levels=c("Actifs ayant un emploi","Étudiants ou élèves","Chômeurs","Inactifs"))) %>%
  summarise(Population = sum(Population)/1000) %>%
  mutate(Population = ceiling(Population)) %>%
  mutate(Année = as.character(Année)) %>%
  ggplot(aes(x=Age,y=Population,fill=Activité,color=Activité)) +
  geom_col(alpha=0.5, color="white", width=1) +
  expand_limits(y=0) +
  scale_fill_manual(values=activités.palette[c(1,4,2,3)]) +
  scale_color_manual(values=activités.palette) +
  theme_cpesr_cap() +
  ggtitle("Activité des français entre 15 et 29 ans en {closest_state}") +
  transition_states(Année) 
```

```{r saphire.allpop.gif.col.amort.20.29, cache=TRUE, fig.asp=9/16}
library(gganimate)
...
saphire %>%
  ...
  group_by(Année,Age,Activité) %>%
  mutate(Activité = fct_recode(Activité, "Chômeurs et inactifs" = "Inactifs", "Chômeurs et inactifs" = "Chômeurs")) %>%
  mutate(Activité=factor(Activité, levels=rev(c("Actifs ayant un emploi","Étudiants ou élèves","Chômeurs et inactifs")))) %>%
  summarise(Population = sum(Population)/1000) %>%
  mutate(Population = ceiling(Population)) %>%
  #mutate(Age = as.character(Age)) %>%
  ggplot(aes(x=Age,y=Population,fill=Activité, group=Activité)) +
  geom_col(alpha=0.5, color="white", width=1, position="fill") +
  geom_hline(yintercept = 1.05, color="grey50") +
  annotate("text",y=1.10,x=scales::rescale(unique(saphire$Année)[-1], from=c(1975,saphire.last.année), to=c(17.5,28.5)),
           label=unique(saphire$Année)[-1], color="grey20", vjust=0) +
  geom_point(aes(x=scales::rescale(Année, from=c(1975,saphire.last.année), to=c(17.5,28.5))), 
             y=1.05, color="grey50", size=3, show.legend = FALSE) +
  expand_limits(y=0) +
  coord_cartesian(clip="off") +
  scale_fill_manual(values=activités.palette[rev(c(1,4,2))]) +
  #scale_color_manual(values=activités.palette) +
  scale_y_continuous(breaks=seq(0,1,0.25), labels=scales::percent, name="Proportion de la population") +
  scale_x_continuous(breaks=seq(18,28)) +
  theme_cpesr_cap() +
  ggtitle("Activité des français entre 18 et 28 ans par âge, en {closest_state}") +
  transition_states(Année) 
```
...
etu <- read.csv2("../data/fr-esr-statistiques-sur-les-effectifs-d-etudiants-inscrits-par-etablissement-hcp.csv") %>%
  select(Rentrée = rentree, starts_with("Cycle.universitaire..cursus.LMD")) %>%
  filter(Rentrée >= 2010) %>%
  pivot_longer(-Rentrée, names_to = "LMD", values_to = "Population") %>%
  mutate(LMD=factor(str_sub(LMD,36,36), levels=c("L","M","D"))) %>%
  group_by(Rentrée, LMD) %>%
  summarise(Population = sum(Population,na.rm = TRUE)) %>%
  group_by(LMD) %>%
  mutate(
    Val100 = Population / first(Population) * 100,
    Evolution = Population - first(Population)) %>%
  mutate(
    Rentrée = factor(Rentrée),
    Cycle = LMD) 
```
...
etu %>%
  ...
  ggplot(aes(x=Rentrée,y=Population,fill=Cycle, group=Cycle)) +
  geom_area(color="white") +
  ylab("Effectifs étudiants (milliers)") +
  theme_cpesr_cap(source="DataESR")
```

```{r sise.pyramide.val100}
etu %>%
  ggplot(aes(x=Rentrée,y=Val100,color=Cycle, fill=Cycle, group=Cycle)) +
  geom_line(size=1) + geom_point(size=4, shape=21, color="white") +
  ylab("Etudiants (valeur 100 en 2010)") +
  theme_cpesr_cap(source="DataESR") +
  ggtitle("Evolution des effectifs étudiants du MESRI par cycle de formation")
```


```{r sise.pyramide.evol}
etu %>%
  mutate(Evolution = Evolution / 1e3) %>%
  ggplot(aes(x=Rentrée,y=Evolution,color=Cycle, fill=Cycle, group=Cycle)) +
  geom_line(size=1) + geom_point(size=4, shape=21, color="white", stroke=1) +
  ylab("Evolution (milliers à partir de 2010)") +
  theme_cpesr_cap(source="DataESR") +
  ggtitle("Evolution des effectifs étudiants du MESRI par cycle de formation")
```

```{r sise.pyramide.evol.data, results='asis'}
etu %>%
  mutate(Evolution = Evolution / 1e3) %>%
  spoilerTable()
```
...
```{r sise.pyramide.col}
etu %>%
  mutate(Evolution = (Population - lag(Population)) / 1e3) %>%
  ggplot(aes(x=Rentrée,y=Evolution,color=Cycle, fill=Cycle, group=Cycle)) +
  geom_col(position="dodge") +
  ylab("Evolution (milliers)") +
  theme_cpesr_cap(source="DataESR") +
  ggtitle("Evolution des effectifs étudiants du MESRI par cycle de formation")
```
