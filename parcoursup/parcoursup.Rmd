...
ps21 <- read.csv2("../data/fr-esr-parcoursup-2021.csv",dec='.') %>%
  rename(
    Libellé.formation = Filière.de.formation.détaillée,
    Filière.de.formation.détaillée = Filière.de.formation.détaillée.bis
  ) 

ps <- bind_rows(apb,ps18,ps19,ps20,ps21) %>%
  ...
  mutate(taux.accès = Taux.d.accès.des.candidats.ayant.postulé.à.la.formation..ratio.entre.le.dernier.appelé.et.le.nombre.vœux.PP. / 100)
```
...
```{r Licence}
licence <- ps %>% filter(Filière.de.formation.très.agrégée %in% c("Licence","Licence_Las")) %>%
  ...
  mutate(Sélectivité = cut(Taux.sélection, 
                           breaks = c(0,0.05,0.10,0.20,0.50,0.80,0.90,0.95,1.00,1000))) %>%
  mutate(Accessibilité = cut(taux.accès, 
  ...
  select(Session,Établissement,Filière.de.formation,Filière.de.formation.très.agrégée,Filière.de.formation.détaillée,
         places, candidats, propositions, admis, rdda, taux.accès, Accessibilité, Taux.sélection, Sélectivité)
