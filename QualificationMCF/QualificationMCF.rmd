...
cnu %>%
  ...
  pivot_longer(-Annee) %>%
  ggplot(aes(x=Annee, y=value, color=name, group=name)) +
    geom_line(size=2) +
...
cnu %>%
  ...
  transmute(
    Annee = Annee,
    evolution.nb.dossiers.MCF = nb.dossiers.MCF / first(nb.dossiers.MCF),
    evolution.nb.qualifications.MCF = nb.qualifications.MCF / first(nb.qualifications.MCF)
  ) %>%
  pivot_longer(-Annee) %>%
  ggplot(aes(x=Annee, y=value, color=name, group=name)) +
    geom_line(size=2) +
