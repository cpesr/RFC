...
cy.long <- cy %>%
  ...
  mutate(
    Part = Montant / Produits.de.fonctionnement.encaissables,
    PartSCSP = Montant / (Produits.de.fonctionnement.encaissables-Ressources.propres.encaissables)
    )
...
```{r}
cy.long %>%
  ggplot(aes(x=exercice, y=Montant, color=Indicateur)) + geom_line() + geom_point() +
...
```{r}
cy.long %>%
  ggplot(aes(x=exercice, y=Part, color=Indicateur)) + geom_line() + geom_point() +
  facet_wrap(~Indicateur, labeller = labeller(Indicateur = label_wrap_gen(25))) +
...
```{r}
cy.long %>%
  ggplot(aes(x=exercice, y=PartSCSP, color=Indicateur)) + geom_line() + geom_point() +
  facet_wrap(~Indicateur, labeller = labeller(Indicateur = label_wrap_gen(25))) +
