---
title: "SCSP"
author: "CPESR"
date: "`r Sys.Date()`"
output: github_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
options(dplyr.summarise.inform = FALSE)
knitr::opts_chunk$set(fig.asp = 9/16, fig.retina = 2)

library(tidyverse)
library(ggcpesrthemes)
theme_set(theme_cpesr() + theme(plot.title = element_text(hjust=1)))
theme_cpesr_setup(authors = "Julien Gossa", source = "https://github.com/cpesr/RFC/")

library(kpiESR)
```

## Données 

- https://github.com/cpesr/kpiESR
- https://data.enseignementsup-recherche.gouv.fr/explore/assets/fr-esr-operateurs-indicateurs-financiers/
- https://www.insee.fr/fr/statistiques/serie/001763852

```{r IPC}
ipc <- read.csv2("../data/IPC.csv", skip=3, dec=".") 
```


```{r scsp, fig.asp=4/3, results='asis'}
Rentrée.max.SCSP <- max(esr %>%
  filter(!is.na(kpi.FIN.S.SCSP)) %>%
  pull(Rentrée))

Rentrée.max.EEC <- max(esr %>%
  filter(!is.na(kpi.ENS.P.effectif)) %>%
  pull(Rentrée))

Rentrée.max.Etu <- max(esr %>%
  filter(!is.na(kpi.ETU.P.effectif)) %>%
  pull(Rentrée))


Rentrée.max <- Rentrée.max.SCSP
Rentrée.min <- 2014
exercice <- Rentrée.max + 1

ipc.val2015 <- ipc %>% filter(Période == paste0(exercice,"-09")) %>% pull(X) 

rangs <- esr %>%
  filter(Rentrée > 2013, Groupe == "Universités et assimilés") %>%
  left_join(esr.etab) %>%
  fill(kpi.ENS.P.effectif) %>%
  transmute(
    Rentrée,
    Exercice = Rentrée + 1,
    Etablissement,
    Typologie,
    PerimEx,
    SCSP = kpi.FIN.S.SCSP,
    SCSP_EEC = kpi.FIN.S.SCSP / kpi.ENS.P.effectif,
    SCSP_Etu = kpi.FIN.S.SCSP / kpi.ETU.P.effectif,
    SCSP_Etu_HorsDoc = kpi.FIN.S.SCSP / (kpi.ETU.S.cycle1_L+kpi.ETU.S.cycle2_M),
    SCSPRessources_Etu_HorsDoc = kpi.FIN.P.ressources / (kpi.ETU.S.cycle1_L+kpi.ETU.S.cycle2_M)
  ) %>%
  pivot_longer(starts_with("SCSP"), names_to = "Indicateur", values_to = "Valeur") %>%
  filter(!is.na(Valeur)) %>% 
  filter(Rentrée %in% c(Rentrée.min,Rentrée.max)) %>%
  arrange(Rentrée) %>% 
  summarise(
    Valeur100 = last(Valeur) / first(Valeur) * 100,
    Valeur = last(Valeur),
    Rentrée.référence = first(Rentrée),
    Rentrée = last(Rentrée),
    .by = c(Etablissement, Typologie, PerimEx, Indicateur)
  ) %>% 
  mutate(Valeur100 = ifelse(Rentrée.référence == Rentrée.min,Valeur100,NA)) %>%
  mutate(Rang = rank(-Valeur), Rang_Valeur100 = rank(-Valeur100), .by=Indicateur) %>%
  mutate(Rang_Typologie = rank(-Valeur), .by=c(Indicateur,Typologie)) %>%
  mutate(Indicateur = factor(Indicateur, 
    levels = c("SCSP","SCSP_EEC","SCSP_Etu","SCSP_Etu_HorsDoc","SCSPRessources_Etu_HorsDoc"),
    labels  = c("SCSP","SCSP/E-EC","SCSP/Etudiant","SCSP/Etudiant(hors doc.)","Ressources/Etudiant(hors doc.)"))) %>%
  mutate(PerimEx = factor(PerimEx,
    levels=c("","ISITE","IDEx"),
    labels=c("NINI","ISITE","IDEx"))) 

write_csv(rangs,"fr-cpesr-scsp.csv")
writexl::write_xlsx(rangs,"fr-cpesr-scsp.xlsx")
rangs %>% spoiler_table("Données")

rangs <- rangs %>%
  filter(!Etablissement %in%
           c("Université Paris sciences et lettres",
             #"Université Gustave Eiffel",
             "Université de Bretagne Occidentale",
             "Université de Guyane",
             "Université de La Réunion",
             "Université des Antilles",
             "Université de la Polynésie Française",
             "Université de la Nouvelle-Calédonie",
             "Centre universitaire de Mayotte"))

ln <- scales::label_number(accuracy = 1)

str_label <-function(x) {
  case_when(
    x>1e6 ~ paste(round(x/1e6)),
    x>17e3 ~ paste(round(x/1e3)),
    TRUE ~ ln(x)
  )
}

rankplot <- function(ind) {
  df <- rangs %>% filter(Indicateur == ind) 
    
  max <- max(df$Valeur, na.rm = TRUE)
  
  df %>%
  mutate(EtabRang = paste(Etablissement,"-",Rang)) %>%
  ggplot(aes(y=reorder(EtabRang,-Rang),x=Valeur, fill=Valeur)) +
  geom_col() + geom_point(color="black", shape=3) + 
  geom_text(aes(label=str_label(Valeur)), hjust=1, x = max*1.2) +
  expand_limits(x=max*1.15) +
  scale_x_continuous(labels = ~ str_label(.x), name=ind) +
  scale_y_discrete(name="") +
  scale_fill_distiller(palette = "RdBu", guide = FALSE, direction = 1) 
}

diffplot <- function(ind) {
  df <- rangs %>% filter(Indicateur == ind) 
    
  max <- max(df$Valeur100-100, na.rm = TRUE)
  min <- max(df$Valeur100-100, na.rm = TRUE)
  
  df %>%
  filter(!is.na(Valeur100)) %>%
  mutate(EtabRang = paste(Etablissement,"-",rank(-Valeur100))) %>%
  mutate(Valeur100.y = Valeur100 - 100 ) %>%
  ggplot(aes(y=reorder(EtabRang,Valeur100.y),x=Valeur100.y, fill=Valeur100.y)) +
  geom_col() + geom_point(color="black", shape=3) + 
  geom_text(aes(label=str_label(Valeur100)), hjust=1, x = max*1.2) +
  geom_vline(xintercept = 0, color="white") +
  geom_vline(xintercept = ipc.val2015-100) +
  expand_limits(x=max*1.15) +
  scale_x_continuous(labels = ~ str_label(.x+100), name=paste(ind,"(valeur 100 pour 2014)")) +
  scale_y_discrete(name="") +
  scale_fill_distiller(palette = "RdBu", guide = FALSE, direction = 1) 
}
```


Téléchargement : [csv](fr-cpesr-scsp.csv) [xlsx](fr-cpesr-scsp.xlsx)

Années de référence : 

- Indicateurs financiers : `r exercice`
- Effectifs enseignants : `r Rentrée.max.EEC`
- Effectifs étudiants : `r Rentrée.max.Etu`

Dans la suite, les données sont limitées aux établissements en France métropolitaine, sans Paris Sciences et Lettres et Université de Bretagne Occidentale, dont les indicateurs sont trop hors-norme.

Dans les représentations en valeur 100, une barre verticale indique l'inflation (ipc en valeur 2015, à `r ipc.val2015`)


## SCSP

```{r rank.scsp, fig.asp=4/3}
  rankplot("SCSP") +
  ggtitle("Subvention pour charge de service public (SCSP)", subtitle = paste("Exercice comptable",exercice,"en millions d'euros"))
```

```{r val100.scsp, fig.asp=4/3}
  diffplot("SCSP") +
  ggtitle("Subvention pour charge de service public (SCSP)", subtitle = paste("Exercice comptable",exercice,"en valeur 100 pour 2014"))
```

## SCSP par E-EC

```{r rank.scsp_eec, fig.asp=4/3}
  rankplot("SCSP/E-EC") +
  ggtitle("Subvention pour charge de service public (SCSP) par enseignant et/ou chercheur", subtitle = paste("Exercice comptable",exercice,"en milliers d'euros"))
```

```{r val100.scsp_eec, fig.asp=4/3}
  diffplot("SCSP/E-EC") +
  ggtitle("Subvention pour charge de service public (SCSP) par enseignant et/ou chercheur", subtitle = paste("Exercice comptable",exercice,"en valeur 100 pour 2014"))
```


## SCSP par Etudiant


```{r rank.scsp_etu, fig.asp=4/3}
  rankplot("SCSP/Etudiant") +
  ggtitle("Subvention pour charge de service public (SCSP) par étudiant", subtitle = paste("Exercice comptable",exercice,"en milliers d'euros"))
```

```{r val100.scsp_etu, fig.asp=4/3}
  diffplot("SCSP/Etudiant") +
  ggtitle("Subvention pour charge de service public (SCSP) par étudiant", subtitle = paste("Exercice comptable",exercice,"en valeur 100 pour 2014"))
```




## SCSP par Etudiant (hors doctorants)


```{r rank.scsp_etuhd, fig.asp=4/3}
  rankplot("SCSP/Etudiant(hors doc.)") +
  ggtitle("Subvention pour charge de service public (SCSP) par étudiant (hors doc.)", subtitle = paste("Exercice comptable",exercice,"en milliers d'euros"))
```

```{r val100.scsp_etuhd, fig.asp=4/3}
  diffplot("SCSP/Etudiant(hors doc.)") +
  ggtitle("Subvention pour charge de service public (SCSP) par étudiant (hors doc.)", subtitle = paste("Exercice comptable",exercice,"en valeur 100 pour 2014"))
```


## Ressources par Etudiant (hors doctorants)



```{r rank.scspres_etuhd, fig.asp=4/3}
  rankplot("Ressources/Etudiant(hors doc.)") +
  ggtitle("Total des ressources par étudiant (hors doc.)", subtitle = paste("Exercice comptable",exercice,"en milliers d'euros"))
```

```{r val100.scspres_etuhd, fig.asp=4/3}
  diffplot("Ressources/Etudiant(hors doc.)") +
  ggtitle("Total des ressources par étudiant (hors doc.)", subtitle = paste("Exercice comptable",exercice,"en valeur 100 pour 2014"))
```



```{r cmp.typo}
rangs %>%
  filter(!is.na(Typologie)) %>%
  filter(Indicateur != "SCSP") %>%
  mutate(Typologie=recode(Typologie,
    "Université pluridisciplinaire avec santé" = "Univ. pluridisc. avec santé",
    "Université pluridisciplinaire hors santé" = "Univ. pluridisc. hors santé",
    "Université scientifique et/ou médicale"="Univ. scientifique et/ou médicale",
    "Université tertiaire - lettres et sciences Humaines" = "Univ. tertiaire LLA-SHS",
    "Université tertiaire - droit et économie" = "Univ. tertiaire DEG" 
    )) %>%
  ggplot(aes(x=Valeur,y=Typologie,fill=Typologie)) +
  geom_violin() + ggbeeswarm::geom_beeswarm(size=0.2) +
  facet_wrap(Indicateur~., scales = "free_x") +
  scale_x_continuous(labels = ~ paste0(.x/1e3,"k€"), name="", breaks = scales::pretty_breaks()) +
  theme(legend.position = "None", axis.title.x = element_blank()) +
  ggtitle("Quatre indicateurs en fonction de la typologie des établissements")
```


```{r cmp.perimex}
rangs %>%
  filter(!is.na(Typologie)) %>%
  filter(Indicateur != "SCSP") %>%
  ggplot(aes(x=Valeur,y=PerimEx,fill=PerimEx)) +
  geom_violin() + ggbeeswarm::geom_beeswarm(size=0.2) +
  facet_wrap(Indicateur~., scales = "free_x") +
  scale_x_continuous(labels = ~ paste0(.x/1e3,"k€"), name="", breaks = scales::pretty_breaks()) +
  theme(legend.position = "None", axis.title.x = element_blank()) +
  ggtitle("Quatre indicateurs en fonction du périmètre d'excellence (PérimEx)", subtitle = "Universités pluridisciplinaires ou scientifiques")
```


```{r cmp.perimex.typo}
rangs %>%
  filter(!is.na(Typologie)) %>%
  filter(!str_detect(Typologie,"tertiaire")) %>%
  ggplot(aes(x=Valeur,y=Typologie,fill=PerimEx)) +
  geom_violin() + ggbeeswarm::geom_beeswarm(size=0.2) +
  facet_wrap(Indicateur~., scales = "free_x") +
  scale_x_continuous(labels = ~ str_label(.x), name="SCSP") +
  theme(legend.position = "None")
```

## Rangs

```{r rangs, fig.asp=4/2}
l <- levels(rangs$Indicateur)
l <- paste(rep(l,each=2),c(exercice,"évolution"))
rangs %>%
  pivot_longer(starts_with("Valeur")) %>%
  mutate(Rang = rank(-value), .by = c(Indicateur,name)) %>%
  mutate(
    name = recode(name,"Valeur"="2025","Valeur100"="évolution"),
    #ff = recode(name,!!exercice:="bold","évolution"="plain"),
    Indicateur2 = factor(paste(Indicateur,name),levels = l)
    ) %>%
  ggplot(aes(x=Indicateur2,y=reorder(Etablissement, value, FUN=max),color=Rang)) +
  geom_point(size=6,shape=15) +
  geom_text(aes(label=Rang) , color="black", size=3.5) +
  scale_color_distiller(palette = "RdBu", guide = FALSE, direction = -1) +
  theme(axis.text.x = element_text(angle=35,hjust=1,vjust=1), 
        axis.title.x = element_blank(), axis.title.y = element_blank()) +
  ggtitle("Classement des universités selon différents indicateurs SCSP", subtitle = paste("Exercice comptable",exercice,"et évolution depuis 2014"))
```


```{r rangs.perimex, fig.asp=4/2}
rangs %>%
  pivot_longer(starts_with("Valeur")) %>%
  mutate(Rang = rank(-value), .by = c(Indicateur,name,PerimEx)) %>%
  mutate(RangNorm = scales::rescale(Rang), .by = c(Indicateur,name,PerimEx)) %>%
  mutate(
    name = recode(name,"Valeur"="2025","Valeur100"="évolution"),
    #ff = recode(name,"2022-2023"="bold","évolution"="plain"),
    Indicateur2 = factor(paste(Indicateur,name),levels = l)
    ) %>%
  ggplot(aes(x=Indicateur2,y=reorder(Etablissement, value, FUN=max),color=RangNorm)) +
  geom_point(size=6,shape=15) +
  geom_text(aes(label=Rang), color="black", size=3.5) +
  scale_color_distiller(palette = "RdBu", guide = FALSE, direction = -1) +
  facet_grid(PerimEx~., scales = "free_y", space = "free") +
  theme(axis.text.x = element_text(angle=35,hjust=1,vjust=1), 
        axis.title.x = element_blank(), axis.title.y = element_blank()) +
  ggtitle("Classement des universités selon différents indicateurs SCSP", subtitle = paste("Exercice comptable",exercice,"et évolution depuis 2014, par PérimEx"))
```


```{r rangs.typo, fig.asp=4/2}

rangs %>%
  pivot_longer(starts_with("Valeur")) %>%
  mutate(Rang = rank(-value), .by = c(Indicateur,name,Typologie)) %>%
  mutate(RangNorm = scales::rescale(Rang), .by = c(Indicateur,name,Typologie)) %>%
  mutate(
    name = recode(name,"Valeur"="2025","Valeur100"="évolution"),
    #ff = recode(name,"2022-2023"="bold","évolution"="plain"),
    Indicateur2 = factor(paste(Indicateur,name),levels = l)
    ) %>%
  mutate(Typologie=recode(Typologie,
    "Université scientifique et/ou médicale"="Univ. scientifique\net/ou médicale",
    "Université tertiaire - lettres et sciences Humaines" = "Univ. tertiaire\nLLA-SHS",
    "Université tertiaire - droit et économie" = "Univ. tertiaire\ndroit et éco." 
    )) %>%
  ggplot(aes(x=Indicateur2,y=reorder(Etablissement, value, FUN=max),color=RangNorm)) +
  geom_point(size=6,shape=15) +
  geom_text(aes(label=Rang), color="black", size=3.5) +
  scale_color_distiller(palette = "RdBu", guide = FALSE, direction = -1) +
  facet_grid(Typologie~., scales = "free_y", space = "free") +
  theme(axis.text.x = element_text(angle=35,hjust=1,vjust=1), 
        axis.title.x = element_blank(), axis.title.y = element_blank()) +
  ggtitle("Classement des universités selon différents indicateurs SCSP", subtitle = paste("Exercice comptable",exercice,"et évolution depuis 2014, par typologie"))
```


## SCSP vs. MS

```{r}
budget <- read.csv2("../data/fr-cpesr-budget-national.csv") 

budget.pnl <- budget %>%
  mutate(Rentrée = Année - 1) %>%
  select(-c(Année, déflateur)) %>%
  pivot_longer(-Rentrée, names_to = "kpi", values_to = "valeur") %>%
  mutate(valeur = valeur * 1e9)
  
```

```{r scsp.vs.ms, fig.asp=9/16}
kpiESR::esr.pnl %>%
  #filter(pid=="Universités et assimilés", Rentrée > 2013) %>%
  filter(pid=="Ensemble", Rentrée > 2015) %>%
  filter(kpi %in% c("kpi.FIN.S.SCSP","kpi.FIN.S.masseSalariale")) %>%
  ggplot(aes(x=Rentrée,y=valeur,color=kpi)) +
  geom_line(size=1) + geom_point(shape = 21, stroke=2,size=2,fill="white") +
  scale_y_continuous(labels = ~ paste(.x/1e9,"Md€"), name = "") +
  scale_color_discrete(labels=c("Masse salariale","SCSP"), name="Indicateur") +
  ggtitle("Dotation récurrente (SCSP) et masse salariale des établissement du MESR")
```

```{r scsp.vs.ms.bis, fig.asp=9/16}
kpiESR::esr.pnl %>%
  #filter(pid=="Universités et assimilés", Rentrée > 2013) %>%
  filter(pid=="Ensemble") %>%
  filter(kpi %in% c("kpi.FIN.S.SCSP","kpi.FIN.S.masseSalariale")) %>%
  bind_rows(budget.pnl %>% filter(kpi == "CASPension")) %>%
  filter(Rentrée > 2015) %>%
  arrange(Rentrée) %>%
  mutate(Evolution = valeur - first(valeur), .by = kpi) %>%
  ggplot(aes(x=Rentrée,y=Evolution,fill=kpi)) +
  geom_col(position="dodge") +
  scale_y_continuous(labels = ~ paste(.x/1e9,"Md€"), name = "") +
  #scale_color_discrete(labels=c("Masse salariale","SCSP"), name="Indicateur") +
  ggtitle("Dotation récurrente (SCSP) et masse salariale des établissement du MESR")
```

```{r scsp.vs.ms2, fig.asp=9/16}
kpiESR::esr %>%
  #filter(pid=="Universités et assimilés", Rentrée > 2013) %>%
  filter(pid=="Ensemble", Rentrée > 2008) %>%
  mutate(taux = kpi.FIN.S.SCSP/kpi.FIN.S.masseSalariale) %>% 
  mutate(Exercice = Rentrée + 1) %>%
  ggplot(aes(x=Exercice,y=taux, color=taux)) +
  geom_hline(yintercept = 1) +
  geom_line(size=1) + geom_point(shape = 21, stroke=2,size=2,fill="white") +
  scale_y_continuous(labels = scales::percent, name = "") +
  scale_color_distiller(palette="Reds") +
  ggtitle("Ratio SCSP / masse salariale des établissement du MESR") +
  theme(legend.position = "None")
```


```{r scsp.vs.ms3, fig.asp=9/16}
kpiESR::esr %>%
  filter(pid=="Ensemble", Rentrée > 2008) %>%
  #filter(pid=="Ensemble", Rentrée > 2008) %>%
  mutate(diff = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale) %>% 
  mutate(Exercice = Rentrée + 1) %>%
  ggplot(aes(x=Exercice,y=diff)) +
  geom_hline(yintercept = 1) +
  geom_line(size=1, color="blue") + geom_point(shape = 21, stroke=2,size=2,fill="white",color="blue") +
  scale_x_continuous(breaks = seq(2009,2024)) +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M€"), name = "SCSP-MS") +
  #scale_color_distiller(palette="Reds") +
  ggtitle("Différence entre SCSP et masse salariale des établissements du MESR") +
  theme(legend.position = "None", panel.grid.minor.x = element_blank())
```

```{r scsp.vs.ms3, fig.asp=9/16}
kpiESR::esr %>%
  filter(pid=="Ensemble", Rentrée > 2008) %>%
  #filter(pid=="Ensemble", Rentrée > 2008) %>%
  mutate(diff = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale) %>% 
  mutate(Exercice = Rentrée + 1) %>%
  
  ggplot(aes(x=Exercice,y=diff)) +
  geom_hline(yintercept = 1) +
  geom_line(size=1, color="blue") + geom_point(shape = 21, stroke=2,size=2,fill="white",color="blue") +
  scale_x_continuous(breaks = seq(2009,2024)) +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M€"), name = "SCSP-MS") +
  #scale_color_distiller(palette="Reds") +
  ggtitle("Différence entre SCSP et masse salariale des établissements du MESR") +
  theme(legend.position = "None", panel.grid.minor.x = element_blank())
```

```{r scsp.vs.ms4, fig.asp=9/16}
kpiESR::esr %>%
  filter(pid=="Universités et assimilés", Rentrée > 2008) %>%
  #filter(pid=="Ensemble", Rentrée > 2008) %>%
  mutate(
    Valeur = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale,
    Série = "Différence SCSP-MS") %>% 
  mutate(Exercice = Rentrée + 1) %>%
  bind_rows(
    budget %>%
      transmute(
        Exercice = Année,
        Valeur = CASPension * 1e9,
        Série = "CAS Pension"
      )
  ) %>%
  ggplot(aes(x=Exercice,y=Valeur, fill=Série)) +
  geom_hline(yintercept = 1) +
  geom_col(position="dodge") +
  #scale_x_continuous(breaks = seq(2009,2026)) +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M€"), name = "SCSP-MS") +
  ggtitle("Différence entre SCSP et masse salariale des universités") +
  theme(legend.position = "None", panel.grid.minor.x = element_blank())
```


```{r scsp.vs.ms5, fig.asp=9/16}
kpiESR::esr.pnl %>%
  #filter(pid=="Universités et assimilés", Rentrée > 2008) %>%
  filter(pid=="Ensemble", Rentrée > 2008) %>%
  filter(kpi %in% c("kpi.FIN.S.SCSP","kpi.FIN.S.masseSalariale")) %>% 
  mutate(kpi = recode(kpi, "kpi.FIN.S.SCSP" = "SCSP", "kpi.FIN.S.masseSalariale" = "Masse Salariale")) %>%
  mutate(Exercice = Rentrée + 1) %>%
  ggplot(aes(x=Exercice,y=valeur, color=kpi)) +
  geom_hline(yintercept = 1) +
  geom_line(size=1) + geom_point(shape = 21, stroke=2,size=2,fill="white") +
  scale_x_continuous(breaks = seq(2009,2024)) +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M€"), name = "SCSP-MS") +
  #scale_color_distiller(palette="Reds") +
  ggtitle("Différence entre SCSP et masse salariale des universités") +
  theme(panel.grid.minor.x = element_blank())
```


### Autres indicateurs

```{r fdr}
fin <- read.csv2("../data/fr-esr-operateurs-indicateurs-financiers.csv", dec='.') %>%
  rename(Etablissement = Nom.Etablissement)

fin %>% 
  filter(str_detect(Etablissement, "niversité")) %>% 
  summarize(Fonds.de.roulement.net.global = sum(Fonds.de.roulement.net.global, na.rm = TRUE), .by = exercice) %>% 
  arrange(exercice) %>% 
  filter(exercice > 2014) %>% 
  ggplot(aes(x=exercice,y=Fonds.de.roulement.net.global)) + geom_line() + geom_point(shape=21, fill="white", stroke=1) + theme_cpesr()
```

```{r scsp.vs.anr}
fin %>% 
  filter(str_detect(Etablissement, "niversité")) %>% 
  transmute(
    exercice,#Etablissement,
    SCSP = Produits.de.fonctionnement.encaissables - Ressources.propres.encaissables,
    ANR.hors.investissements.d.avenir,
    ANR.investissements.d.avenir) %>%
  pivot_longer(-exercice, names_to = "Budget", values_to = "Valeur") %>%
  summarise(Valeur = sum(Valeur, na.rm = TRUE), .by = c(exercice,Budget)) %>%
  ggplot(aes(x=exercice,y=Valeur,fill=Budget)) + geom_area()
```


```{r deficit, fig.asp=4/3}
fin %>% 
  filter(exercice == max(exercice)) %>%
  left_join(kpiESR::esr.etab, by = join_by(id...paysage == pid)) %>% 
  filter(Groupe == "Universités et assimilés") %>% 
  mutate(Etablissement = coalesce(Etablissement.x, Etablissement.y)) %>%
  ggplot(aes(x=Résultat.net.comptable,y=reorder(Etablissement,Résultat.net.comptable),fill=PerimEx)) + 
  geom_col() +
  ylab(NULL) +
  scale_x_continuous(labels = ~ paste(.x/1e6,"M€"), name="Résultat net comptable") +
  ggtitle("Résultat net comptable des universités", subtitle = "2024, en millions d'euros")
```

```{r deficit2, fig.asp=4/3}
fin %>% 
  filter(exercice == max(exercice)) %>%
  left_join(kpiESR::esr.etab, by = join_by(id...paysage == pid)) %>% 
  filter(Groupe == "Universités et assimilés") %>% 
  mutate(Etablissement = coalesce(Etablissement.x, Etablissement.y)) %>%
  mutate(PartDeficit = Résultat.net.comptable / Produits.de.fonctionnement.encaissables) %>%
  filter(!str_detect(Etablissement,"Lumière")) %>%
  ggplot(aes(x=PartDeficit,y=reorder(Etablissement,PartDeficit),fill=PerimEx)) + geom_col()
```



## SCSCP vs RP

```{r scspvsrp, fig.asp=3/4}
fin %>% 
  filter(str_detect(Etablissement, "niversité")) %>% 
  summarize(
    Ressources = sum(Produits.de.fonctionnement.encaissables, na.rm = TRUE),
    Ressources.propres = sum(Ressources.propres.encaissables, na.rm = TRUE),
    .by = exercice) %>% 
  mutate(Proportion = Ressources.propres / Ressources) %>%
  pivot_longer(-exercice, names_to = "Indicateur", values_to = "Valeur") %>%
  ggplot(aes(x=exercice,y=Valeur,color=Indicateur)) + 
  geom_vline(xintercept = c(2014,2020), linetype="dashed", color="grey") +
  geom_line() + geom_point(shape=21, fill="white", stroke=1) + 
  facet_grid(Indicateur~., scales = "free_y") +
  scale_y_continuous(labels = ~ ifelse(.x<1,scales::percent(.x), paste(.x/1e9,"Md€")), name="") +
  theme_cpesr()
```


## Fix temporaire 

```{r}
  fin <- read.csv2("../data/fr-esr-operateurs-indicateurs-financiers.csv", na.strings="", dec=".") %>%
    mutate(
      Etablissement = Nom.Etablissement,
      pid = id...paysage,
      exercice,
      Charges.de.personnel = case_when(
      is.na(Charges.de.personnel) ~ Charges.de.personnel...Produits.encaissables * Produits.de.fonctionnement.encaissables / 100,
      TRUE ~ Charges.de.personnel
    )) %>%
    summarise(
      kpi.FIN.P.ressources = sum(Produits.de.fonctionnement.encaissables),
      kpi.FIN.S.masseSalariale = sum(Charges.de.personnel),
      kpi.FIN.S.SCSP = sum(kpi.FIN.P.ressources) - sum(Ressources.propres.encaissables),
      kpi.FIN.S.recettesFormation = sum(Droits.d.inscription) +
                                    sum(Formation.continue..diplômes.propres.et.VAE) +
                                    sum(Taxe.d.apprentissage),
      kpi.FIN.S.recettesRecherche = sum(Valorisation, na.rm = TRUE) +
                                    sum(ANR.hors.investissements.d.avenir) +
                                    sum(ANR.investissements.d.avenir) +
                                    sum(Contrats.et.prestations.de.recherche.hors.ANR, na.rm = TRUE),
      kpi.FIN.S.investissements = sum(Acquisitions.d.immobilisations),
      Solde.budgétaire = sum(Solde.budgétaire),
      Résultat.net.comptable = sum(Résultat.net.comptable),
      FDR = sum(Fonds.de.roulement.en.jours.de.fonctionnement),
      Trésorerie = sum(Trésorerie),
      .by=c(pid,Etablissement,exercice)
    ) %>%
    ungroup() %>%
    arrange(pid,exercice)

# fin <- left_join(fin,kpiESR::esr.etab %>% filter(dataset == "etab.25"))

fin %>%
  filter(exercice > 2008) %>%
  mutate(diff = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale) %>% 
  mutate(Exercice = exercice) %>%
  summarize(diff = sum(diff,na.rm=TRUE), .by=Exercice) %>%
  ggplot(aes(x=Exercice,y=diff, color=diff)) +
  geom_hline(yintercept = 1) +
  geom_line(size=1) + geom_point(shape = 21, stroke=2,size=2,fill="white") +
  scale_x_continuous(breaks = seq(2009,2024)) +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M€"), name = "SCSP-MS") +
  scale_color_distiller(palette="Reds") +
  ggtitle("Différence entre SCSP et masse salariale des établissements du MESR") +
  theme(legend.position = "None", panel.grid.minor.x = element_blank())

```

```{r}
fin %>%
  filter(exercice > 2008, str_detect(Etablissement, "niversit")) %>%
  mutate(Exercice = exercice) %>%
  summarize(Trésorerie = sum(Trésorerie,na.rm=TRUE), .by=Exercice) %>%
  ggplot(aes(x=Exercice,y=Trésorerie, color=Trésorerie)) +
  geom_hline(yintercept = 1) +
  geom_line(size=1) + geom_point(shape = 21, stroke=2,size=2,fill="white") +
  scale_x_continuous(breaks = seq(2009,2024)) +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M€"), name = "SCSP-MS") +
  scale_color_distiller(palette="Reds") +
  ggtitle("Différence entre SCSP et masse salariale des établissements du MESR") +
  theme(legend.position = "None", panel.grid.minor.x = element_blank())
```



```{r}
fin %>%
  filter(exercice == 2025) %>%
  mutate(diff = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale) %>% 
  ggplot(aes(x=kpi.FIN.S.SCSP,y=kpi.FIN.S.masseSalariale, color=diff)) +
  geom_abline(intercept = 0, slope =1) +
  geom_point(shape = 21, stroke=2,size=2,fill="white") +
  scale_color_distiller(palette="Reds") +
  ggtitle("Différence entre SCSP et masse salariale des établissements du MESR") +
  theme(legend.position = "None", panel.grid.minor.x = element_blank())
```

```{r}
fin %>%
  filter(exercice == 2025, str_detect(Etablissement,"niversi")) %>%
  left_join(kpiESR::esr.etab %>% filter(dataset=="etab.25", PerimEx != "") %>% rename(etab = Etablissement)) %>%
  replace_na(list(PerimEx = "NINI")) %>%
  mutate(
    diff = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale,
    TauxSCSPMS = kpi.FIN.S.SCSP / kpi.FIN.S.masseSalariale,
    TauxSCSP = kpi.FIN.S.SCSP / kpi.FIN.P.ressources,
    TauxMS = kpi.FIN.S.masseSalariale / kpi.FIN.P.ressources
    ) %>% 
  filter(TauxSCSP>0.5, TauxSCSPMS<1.2) %>%
  ggplot(aes(x=kpi.FIN.P.ressources,y=diff, color=PerimEx)) +
  #geom_hline(yintercept = 1) +
  geom_point(shape = 21, stroke=2,size=2,fill="white") +
  #scale_color_distiller(palette="Reds") +
  ggtitle("Différence entre SCSP et masse salariale des établissements du MESR") 
```

```{r}
fin %>%
  filter(exercice == 2025, str_detect(Etablissement,"niversi")) %>%
  left_join(kpiESR::esr.etab %>% filter(dataset=="etab.25", PerimEx != "") %>% rename(etab = Etablissement)) %>%
  replace_na(list(PerimEx = "NINI")) %>%
  mutate(
    diff = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale,
    TauxSCSPMS = kpi.FIN.S.SCSP / kpi.FIN.S.masseSalariale,
    TauxSCSP = kpi.FIN.S.SCSP / kpi.FIN.P.ressources,
    TauxMS = kpi.FIN.S.masseSalariale / kpi.FIN.P.ressources
    ) %>% 
  filter(TauxSCSP>0.5, TauxSCSPMS<1.2) %>%
  ggplot(aes(x=kpi.FIN.P.ressources,y=TauxSCSPMS, color=PerimEx)) +
  geom_hline(yintercept = 1) +
  geom_point(shape = 21, stroke=2,size=2,fill="white") +
  #scale_color_distiller(palette="Reds") +
  ggtitle("Différence entre SCSP et masse salariale des établissements du MESR") 
```



```{r}
fin %>%
  filter(exercice == 2025, str_detect(Etablissement,"niversi")) %>%
  left_join(kpiESR::esr.etab %>% filter(dataset=="etab.25", PerimEx != "") %>% rename(etab = Etablissement)) %>%
  replace_na(list(PerimEx = "NINI")) %>%
  mutate(
    diff = kpi.FIN.S.SCSP-kpi.FIN.S.masseSalariale,
    TauxSCSPMS = kpi.FIN.S.SCSP / kpi.FIN.S.masseSalariale,
    TauxSCSP = kpi.FIN.S.SCSP / kpi.FIN.P.ressources,
    TauxMS = kpi.FIN.S.masseSalariale / kpi.FIN.P.ressources
    ) %>% 
  filter(TauxSCSP>0.5, TauxSCSPMS<1.2) %>%
  ggplot(aes(x=kpi.FIN.P.ressources,y=Solde.budgétaire, color=PerimEx)) +
  #geom_hline(yintercept = 1) +
  geom_point(shape = 21, stroke=2,size=2,fill="white") +
  #scale_color_distiller(palette="Reds") +
  scale_x_continuous(labels = ~ paste(.x/1e6,"M€"), name = "Ressources totales (M€)") +
  scale_y_continuous(labels = ~ paste(.x/1e6,"M€"), name = "Solde budgétaire (M€)") +
  ggtitle("Différence entre SCSP et masse salariale des établissements du MESR") +
  theme(legend.position = "right")
```



```{r, cache = TRUE, eval=FALSE}
library(gganimate)

p <- fin %>%
  filter(exercice >= 2010) %>%
         #str_detect(Etablissement, "niversi")) %>%
  left_join(
    kpiESR::esr.etab %>%
      filter(dataset == "etab.25", PerimEx != "") %>%
      rename(etab = Etablissement)
  ) %>%
  replace_na(list(PerimEx = "NINI")) %>%
  mutate(
    diff = kpi.FIN.S.SCSP - kpi.FIN.S.masseSalariale,
    TauxSCSPMS = kpi.FIN.S.SCSP / kpi.FIN.S.masseSalariale,
    TauxSCSP   = kpi.FIN.S.SCSP / kpi.FIN.P.ressources,
    TauxMS     = kpi.FIN.S.masseSalariale / kpi.FIN.P.ressources
  ) %>%
  filter(TauxSCSP > 0.5, TauxSCSPMS < 1.2) %>%
  ggplot(aes(x = kpi.FIN.P.ressources, y = Résultat.net.comptable, color = PerimEx)) +
  geom_smooth(method="lm", color = "grey") +
  geom_point(aes(group = pid), shape = 21, stroke = 2, size = 2, fill = "white") +
  scale_x_continuous(labels = ~ paste0(.x/1e6, " M€"), name = "Ressources totales (M€)") +
  scale_y_continuous(labels = ~ paste0(.x/1e6, " M€"), name = "Résultat net comptable (M€)") +
  labs(color = "Périmètre") +
  ggtitle("Résultats nets comptables des établissements MESR", subtitle="Exercice : {closest_state}") +
  theme(legend.position = "bottom", base_size = 32) +
  transition_states(exercice, state_length = 1, wrap = FALSE) +
  ease_aes("cubic-in-out")

anim <- animate(p, fps = 6, width = 1024, height = 1024, res = 200, end_pause = 10, renderer = gifski_renderer())

anim_save("scsp_ms_2009_2025.gif", anim)
```