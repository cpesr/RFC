...
delta.angle <- function(angle, anglecut=0) {
  read.table("fr-esr-statistiques-sur-les-effectifs-d-etudiants-inscrits-par-etablissement.csv",
    ...
    group_by(name) %>%
    mutate(Evolution = Etudiants - lag(Etudiants)) %>%
    mutate(RENTREE = factor(RENTREE, levels=c("2015","2014","2013"))) %>%
    #filter(RENTREE == 2015) %>%
    ggplot(aes(x=reorder(name,Evolution), y=Evolution, fill=RENTREE)) +
      geom_col() +
      coord_flip() +
      xlab("") +
...
La hausse des effectifs étudiants entre 2010 et 2015 semble réelle, avec un effet de pic dû notamment à une baisse en Master d'enseignement en 2014, conséquente de la formation des maîtres, 'abord par l'annulation de la "masterisation" en 2012, puis la transformation des IUFM en ESPE en 2013.