...
read.table("fr-esr-statistiques-sur-les-effectifs-d-etudiants-inscrits-par-etablissement.csv",
  ...
  select(RENTREE, starts_with("Bac")) %>% 
  pivot_longer(-RENTREE, names_to="Bac") %>%
  group_by(RENTREE,Bac) %>%
  summarise(Etudiants = sum(value)) %>%
  ggplot(aes(x=RENTREE, y=Etudiants, color=Bac, group=Bac)) +
    geom_line() +
    theme_cpesr_cap()
```
...
read.table("fr-esr-statistiques-sur-les-effectifs-d-etudiants-inscrits-par-etablissement.csv",
  ...
  mutate(name = substr(name,7,100)) %>%
  group_by(RENTREE,name) %>%
  summarise(Etudiants = sum(value, na.rm=TRUE)) %>%
  group_by(name) %>%
  mutate(Evolution = Etudiants - first(Etudiants)) %>%
  filter(RENTREE == 2015) %>%
  ggplot(aes(x=reorder(name,Evolution), y=Evolution, fill=name)) +
    geom_col() +
    coord_flip() +
    xlab("") +
...
read.table("fr-esr-statistiques-sur-les-effectifs-d-etudiants-inscrits-par-etablissement.csv",
                  header=TRUE, sep=';', quote='"') %>%
  filter(Type.d.établissement == "Université", RENTREE %in% c(2014,2015)) %>%
  select(RENTREE, starts_with("Diplôme.préparé")) %>% 
  pivot_longer(-RENTREE) %>%
  mutate(name = substr(name,19,100)) %>%
  group_by(RENTREE,name) %>%
  summarise(Etudiants = sum(value, na.rm=TRUE)) %>%
  group_by(name) %>%
  mutate(Evolution = Etudiants - first(Etudiants)) %>%
...
read.table("fr-esr-statistiques-sur-les-effectifs-d-etudiants-inscrits-par-etablissement.csv",
                  header=TRUE, sep=';', quote='"') %>%
  filter(Type.d.établissement == "Université", RENTREE %in% c(2014,2015)) %>%
  select(RENTREE, starts_with("Discipline")) %>% 
  pivot_longer(-RENTREE) %>%
  mutate(name = substr(name,14,100)) %>%
  group_by(RENTREE,name) %>%
  summarise(Etudiants = sum(value, na.rm=TRUE)) %>%
  group_by(name) %>%
  mutate(Evolution = Etudiants - first(Etudiants)) %>%
...
read.table("fr-esr-statistiques-sur-les-effectifs-d-etudiants-inscrits-par-etablissement.csv",
                  header=TRUE, sep=';', quote='"') %>%
  filter(Type.d.établissement == "Université", RENTREE %in% c(2014,2015)) %>%
  select(RENTREE, starts_with("Secteur.disciplinaire")) %>% 
  pivot_longer(-RENTREE) %>%
  mutate(name = substr(name,25,100)) %>%
  group_by(RENTREE,name) %>%
  summarise(Etudiants = sum(value, na.rm=TRUE)) %>%
  group_by(name) %>%
  mutate(Evolution = Etudiants - first(Etudiants)) %>%
