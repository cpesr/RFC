...
ens <- read.table("fr-esr-enseignants-titulaires-esr-public.csv",
  ...
  transmute(
    ...
    Type.établissement.ens = factor(case_when(
      Type.établissement == "Université" ~ "Université",
      Type.établissement == "Grand établissement" ~ "Grand établissement",
      Type.établissement %in% c("École habilitée à délivrer un diplôme d'ingénieur", "École nationale supérieure d'ingénieurs", "École d'ingénieurs", "École de formation artistique","Institut national polytechnique","École normale supérieure") ~ "École",
      TRUE ~ "Autre établissement")),
    Rentrée = Rentrée,
    Enseignants = effectif
  ) %>%
  group_by(UAI, Etablissement, Région, Type.établissement.ens, Rentrée) %>%
  summarise(Enseignants = sum(Enseignants, na.rm = TRUE)) %>%
  ungroup()

tde <- full_join(ens, etu) %>%
  mutate(Rentrée = as.factor(Rentrée)) %>%
  mutate(Type.établissement = ifelse(is.na(Type.établissement.etu), Type.établissement.ens, Type.établissement.etu)) %>%
  select(-Type.établissement.etu, -Type.établissement.ens)
